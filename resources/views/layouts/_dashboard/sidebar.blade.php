<ul class="navbar-nav bg-gradient-secondary sidebar sidebar-dark accordion" id="accordionSidebar">
    ...
    @canany(['user_show', 'role_show', 'toko_show'])
        <li class="nav-item {{ set_active(['roles*', 'user*', 'setting-toko*']) }}">
            <a class="nav-link {{ request()->routeIs('roles*', 'user*', 'setting-toko*') ? '' : 'collapsed' }}"
                href="#" data-toggle="collapse" data-target="#collapsePages3" aria-expanded="true"
            ...
            <div id="collapsePages3"
                ...
                <div class="bg-white py-2 collapse-inner rounded">
                    @can('user_show')
                        <a class="collapse-item {{ set_active(['user*']) }}" href="{{ route('user.index') }}"> User Admin</a>
                    @endcan
                    @can('role_show')
                        <a class="collapse-item {{ set_active(['roles*']) }}" href="{{ route('roles.index') }}"> Roles</a>
                    @endcan
                    @can('toko_show')
                        <a class="collapse-item {{ set_active(['setting-toko*']) }}" href="{{ route('setting-toko.index') }}">
                            Setting Toko</a>
                    @endcan
                    {{-- @can('belanja_show')
                        <a class="collapse-item {{ set_active(['cara-belanja*']) }}" href="{{ route('cara-belanja.index') }}">
                            Cara Belanja</a>
                    @endcan --}}
                </div>
            </div>
